...
def load_posterior(paper):
    """
    in: paper: Daniel2019 loads the posterior from Gilman et al. 2020 https://arxiv.org/abs/1908.06983
               Nadler2021 load the posterior from Nadler et al. 2021  https//arxiv.org/pdf/2101.07810.pdf
...
def marginalizing_over_sigma(f):
    ### Marginalizing over Sigma_sub
    marginalization_samples = 1000
    ...
    ax.set_aspect(0.01)
#     fig.colorbar(color_map,ax=ax, anchor=(0, 0.5), shrink=0.4)
#     fig.savefig(dark_matter_paper_location+"/joint_posterior.pdf",bbox_inches = 'tight',pad_inches = 0.01)
#     fig.savefig(DARK_MATTER_PLOTS_LOCATION+"/strong_lensing_limits/joint_posterior.jpg",bbox_inches = 'tight',pad_inches = 0.03,dpi=600)
...
def plot_m_hm_posterior(log10_m_hm,p_log10_mhm):
    ...
    fig2.savefig(DARK_MATTER_PLOTS_LOCATION+"/strong_lensing_limits/marginalized_posterior_log.jpg",bbox_inches = 'tight',pad_inches = 0.03,dpi=600)

def mhm_log_to_linear(log10_m_hm,p_log10_mhm):
    #### Moving from log mhm posterior to linear posterior
    m_hm = 10**log10_m_hm
    ...
    # paper_plot = True
    # if paper_plot==True:
    #     fig.savefig(dark_matter_paper_location+"/marginalized_posterior.pdf",bbox_inches = 'tight',pad_inches = 0.01)
    #     fig.savefig(DARK_MATTER_PLOTS_LOCATION+"/strong_lensing_limits/marginalized_posterior.jpg",bbox_inches = 'tight',pad_inches = 0.03,dpi=600)
    return m_hm, p_m_hm